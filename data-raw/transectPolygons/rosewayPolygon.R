...
#roseway line stations, 00, 02 : 08, note that 01 is between BBL and RL
lon <- c(-64.8808, -64.6203, -64.3992, -64.1802, -64.0795, -63.8702, -63.5095, -63.1193)
lat <- c(43.5598, 43.2497, 42.9982, 42.75, 42.6183,  42.3198, 41.8192, 41.2998)

nums <- unlist(lapply(c(0, 2:8), function(k) ifelse(k < 10, paste0(0,k), k)))
names <- paste('RL', nums, sep = '_')
...
rosewayPolygon <- list(longitude = llpolyx,
                          latitude = llpolyy)
usethis::use_data(rosewayPolygon, compress = 'xz', overwrite = TRUE)
...
for(i in 1:length(lon)){
        ...
        stnpoly[[i]][['transectName']] <- 'roseway'

}
rosewayStationPolygons <- stnpoly
usethis::use_data(rosewayStationPolygons, compress = 'xz', overwrite = TRUE)
...
#png('00_rosewayPolygon.png', width = 6, height = 4, unit = 'in', res = 200, pointsize = 12)
